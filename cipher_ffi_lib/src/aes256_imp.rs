...
static mut AES256_CORE: Lazy<Aes256> = Lazy::new(|| Aes256::new(&GenericArray::from([0; 32])));

pub fn aes256_update_key(key: Vec<u8>) {
    let key = GenericArray::from(cast_vector2array32(key));
    let new_core = &Aes256::new(&key);
    unsafe { AES256_CORE.clone_from(new_core) };
}
...
fn encrypt(data: Vec<u8>) -> Vec<u8> {
    ...
    unsafe {
        AES256_CORE.encrypt_block(block);
    };
...
fn decrypt(data: Vec<u8>) -> Vec<u8> {
    ...
    unsafe {
        AES256_CORE.decrypt_block(block);
    };
    cast_array16_2vector(block.as_slice())
}
...
fn cast_vector2array32(vec: Vec<u8>) -> [u8; 32] {
